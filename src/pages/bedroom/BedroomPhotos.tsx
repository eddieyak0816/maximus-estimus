...
import type { BedroomPhotos as BP, CustomPhoto } from '../../types';
...
export default function BedroomPhotos({ data, assessmentId, jobId, onUpdate }: Props) {
  ...
  async function handlePhotoCapture(photoKey: keyof BP, blob: Blob) {
    try {
  ...
  return (
    <div className="assess-tab">
      ...
      <PhotoItem
        label="Closets — full view"
        photoId={data.closets}
        onOpenCamera={() => setActivePhotoKey('closets')}
        onRemove={() => handlePhotoRemove('closets')}
      />
      <PhotoItem
      ...
      {activePhotoKey && (
        <CameraModal
          ...
          onCapture={(blob) => handlePhotoCapture(activePhotoKey as keyof BP, blob)}
